...
function buildMaskPrompt(material: string, maskTitle: string, promptNotes?: string): string {
  return [
    ...
    "",
    "The source texture image is the single source of truth for visual density, feature scale, palette, brushwork, and material detail.",
    "Match the density of the source texture exactly: the amount of grass blades, stones, flowers, ripples, speckles, cracks, glow marks, and other repeated details per tile-sized area must remain equal to the provided texture image.",
    "Do not make any generated mask sheet denser, sparser, larger-scaled, smaller-scaled, smoother, noisier, more detailed, or less detailed than the source texture.",
    "Use text material notes only to understand what the source texture represents; never invent a new density or pattern scale from the text.",
    "All mask sheets generated for this material must look like different masked cutouts of the same seamless texture, not newly reinterpreted textures.",
    "",
