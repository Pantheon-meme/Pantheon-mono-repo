...
import {PlayerState} from "../src/codegen/tables/PlayerState.sol";
...
contract MintDevAgent is Script {
    ...
    function run(address worldAddress) public {
        ...
        _mintAndAuthorize(worldAddress, agentInftAddress, ownerPrivateKey, owner, executorConfig.executor);
    }
    ...
    function _mintAndAuthorize(
        ...
        uint256 tokenId =
            agentInft.mint(_intelligentData(), owner, config.publicURI, config.encryptedStorageURI, config.memoryRoot);
        IWorld(worldAddress).pantheon__registerAgent(tokenId, executor, config.publicName, config.publicURI);
        agentInft.authorizeUsageWithPermissions(tokenId, executor, abi.encode(config.permissionBits), config.expiresAt);
        IWorld(worldAddress).pantheon__mirrorINFTAuthorization(
            tokenId, executor, config.expiresAt, config.maxActionsPerEpoch, config.maxCucSpendPerEpoch
        );
    ...
    function _maybeSpawnExecutor(address worldAddress, ExecutorConfig memory executorConfig) private {
        if (!executorConfig.hasPrivateKey || PlayerState.getExists(executorConfig.executor)) {
            return;
        }
    ...
    function _agentMintConfig() private view returns (AgentMintConfig memory config) {
        config.publicName = _envString("AGENT_PUBLIC_NAME", "Aletheia");
        config.publicURI = _envString("AGENT_PUBLIC_URI", "0g-storage://local-dev/agents/aletheia/public.json");
        config.encryptedStorageURI =
            _envString("AGENT_ENCRYPTED_STORAGE_URI", "0g-storage://local-dev/agents/aletheia/encrypted");
        ...
        config.maxActionsPerEpoch = uint32(vm.envOr("AGENT_MAX_ACTIONS_PER_EPOCH", DEFAULT_MAX_ACTIONS_PER_EPOCH));
        config.maxCucSpendPerEpoch = vm.envOr("AGENT_MAX_CUC_SPEND_PER_EPOCH", DEFAULT_MAX_CUC_SPEND_PER_EPOCH);
    }

    function _intelligentData() private view returns (IntelligentData[] memory intelligentData) {
        ...
        intelligentData[0] = IntelligentData({
            ...
            dataHash: _hashEnvOrUri(
                "AGENT_CAPSULE_HASH", "AGENT_CAPSULE_URI", "0g-storage://local-dev/agents/aletheia/capsule.enc.json"
            )
        });
    ...
    function _agentInftAddress() private view returns (address inftContract) {
        if (vm.envExists("AGENT_INFT_ADDRESS")) {
            ...
            inftContract = AgentConfig.getInftContract(PantheonConstants.AGENT_CONFIG_ID);
        }
    ...
    function _defaultPermissionBits() private pure returns (uint256) {
        return AgentPermissionLib.CAN_RUN_INFERENCE | AgentPermissionLib.CAN_ACT_IN_WORLD | AgentPermissionLib.CAN_MOVE
            | AgentPermissionLib.CAN_FORAGE | AgentPermissionLib.CAN_SLEEP | AgentPermissionLib.CAN_PICKUP
            | AgentPermissionLib.CAN_PLANT | AgentPermissionLib.CAN_HARVEST | AgentPermissionLib.CAN_BANK_SELL
            | AgentPermissionLib.CAN_APPEND_MEMORY;
    }

    function _hashEnvOrUri(string memory hashEnv, string memory uriEnv, string memory defaultURI)
        private
        view
        returns (bytes32)
    {
    ...
    function _envString(string memory key, string memory defaultValue) private view returns (string memory) {
        if (vm.envExists(key)) {
