...
import { buildBlobAtlasSlotLookup, blobTextureKey } from "../autotile/BlobAutotile";
...
export class AutotileRenderSystem implements System {
    ...
    for (const [, grid, layer] of world.query(TerrainGrid, AutotileLayer)) {
      ...
      for (const key of grid.cells) {
        ...
        const sprite = this.scene.add
          .image(x * grid.tileSize, y * grid.tileSize, blobTextureKey(layer.texturePrefix, mask))
          .setOrigin(0)
  ...
  private ensureTileTextures(layer: AutotileLayer): void {
    ...
    for (const [mask, slot] of this.atlasSlots) {
      const source = this.scene.textures.get(layer.atlasKey).getSourceImage() as HTMLImageElement | HTMLCanvasElement;
      ...
      const imageData = context.getImageData(0, 0, layer.sourceTileSize, layer.sourceTileSize);
      ...
      this.scene.textures.addCanvas(blobTextureKey(layer.texturePrefix, mask), canvas);
    }
...
export function getNeighborMask(grid: TerrainGrid, x: number, y: number): NeighborMask {
