...
export class TerrainBackgroundSystem implements System {
  ...
  constructor(private readonly scene: Phaser.Scene) {
    this.nextBackgroundKey = this.scene.input.keyboard?.addKey(Phaser.Input.Keyboard.KeyCodes.B);
  }

  update(world: World): void {
    for (const [, grid, background] of world.query(TerrainGrid, TerrainBackground)) {
  ...
  private updateMode(background: TerrainBackground): void {
    if (!this.nextBackgroundKey || !Phaser.Input.Keyboard.JustDown(this.nextBackgroundKey)) {
      return;
    }
  ...
  private renderBackground(grid: TerrainGrid, background: TerrainBackground): void {
