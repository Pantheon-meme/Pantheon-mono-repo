    ...
    for (const [, grid, layer] of world.query(TerrainGrid, TerrainBaseLayer)) {
      ...
      for (let y = 0; y < grid.height; y += 1) {
        for (let x = 0; x < grid.width; x += 1) {
          const color = hash01(x, y) > 0.68 ? layer.variantColor : layer.baseColor;

          layer.graphics.fillStyle(color, 1);
          layer.graphics.fillRect(x * grid.tileSize, y * grid.tileSize, grid.tileSize, grid.tileSize);
        }
      }
      ...
      for (let x = 0; x <= grid.width; x += 1) {
        layer.graphics.lineBetween(x * grid.tileSize, 0, x * grid.tileSize, worldHeight);
      }

      for (let y = 0; y <= grid.height; y += 1) {
        layer.graphics.lineBetween(0, y * grid.tileSize, worldWidth, y * grid.tileSize);
      }
